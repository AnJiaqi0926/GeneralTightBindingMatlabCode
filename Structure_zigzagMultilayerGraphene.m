function [a1, a2, atom_position] = Structure_zigzagMultilayerGraphene(Nx, Ny, plot_structure,params)
    % 构建石墨烯超胞函数
    %
    % 输入参数:
    ...
    for ix = 0:Nx-1       % 沿a1方向
        for iy = 0:Ny-1   % 沿a2方向
            ...
            for iz = 0:total_layer-1
                ...
                atom_count = atom_count + 1;
                atom_position(atom_count, :) = [...
                    displacement(1) + posC(1) + layer_shift(1), ... % x坐标
                    displacement(2) + posC(2) + layer_shift(2), ... % y坐标
                    3.35*iz, ...                   % z坐标
                    iz + 1, ...                    % 层数
                    1, ...                         % 子晶格 (A=1)
                    1, ...                         % 原子类型
                ...
                atom_count = atom_count + 1;
                atom_position(atom_count, :) = [...
                    displacement(1) + posD(1) + layer_shift(1), ... % x坐标
                    displacement(2) + posD(2) + layer_shift(2), ... % y坐标
                    3.35*iz, ...                   % z坐标
                    iz + 1, ...                    % 层数
                    2, ...                         % 子晶格 (B=2)
                    1, ...                         % 原子类型
...
function layer_shift = getLayerShift(iz, StackedMode, shift)
    ...
    switch StackedMode
        ...
        case 'AB'
            % AB堆叠：交替层偏移
            % 偶数层无偏移，奇数层有偏移
            layer_shift = mod(iz,2) * shift;       % 奇数层
