function [a1, a2, atom_position] = Structure_GBN(Nx, Ny, plot_structure,params)
    % 构建石墨烯超胞函数
    %
    % 输入参数:
    ...
    a1BN_single = [a_BN, 0];                % a1基矢
    a2BN_single = [a_BN/2, a_BN*sqrt(3)/2];    % a2基矢
    ...
    posC = (a1BN_single + a2BN_single)/3;
    posD = 2*(a1BN_single + a2BN_single)/3; 
    ...
    if ifspin
        ...
        total_atoms = 2 * total_layer * Nx * Ny;  % 总原子数 (每个原胞2个原子)
        fprintf('构建 %dx%d 石墨烯超胞: %d 个原子\n', Nx, Ny, total_atoms);
    end
    ...
    for ix = 0:Nx-1       % 沿a1方向
        for iy = 0:Ny-1   % 沿a2方向
            ...
            if ix ~= Nx && iy ~= Ny
                displacement = ix * a1BN_single + iy * a2BN_single;
                % 添加C原子 (子晶格A)
                atom_count = atom_count + 1;
                atom_position(atom_count, :) = [...
