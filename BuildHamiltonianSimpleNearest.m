function [Hamiltonian,neighbor_cell, disp_vec] = BuildHamiltonianSimpleNearest(a1, a2, atom_position, N, neighbor_cell, params)
    % 构建紧束缚模型哈密顿量函数
    %
    % 输入参数:
    %   a1, a2 - 正格子基矢
        ...
        parfor k = 1:N_neighbor
            ...
            for i = 1:N
                for j = 1:N
                    ...
                    elseif abs(layer(i) - layer(j)) == 1 && R_sq < 0.1
                        H_slice(i, j) = H_slice(i, j) + t2;
                    elseif i == j && k == 1 
                        H_slice(i, j) = H_slice(i, j) + layer(i) * E_elec;
                    end
                end
            end
        ...
        for k = 1:N_neighbor
            ...
            for i = 1:N
                for j = 1:N
                    ...
                    elseif i == j && k == 1 
                        Hamiltonian(i, j, k) = Hamiltonian(i, j, k) + layer(i) * E_elec;
                    end
                end
            end
        end
