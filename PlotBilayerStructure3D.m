function PlotBilayerStructure3D(a1, a2, a, atom_position, Nx, Ny, mode)
    ...
    % hBN层 (layer=2)
    hBN_N_idx = (layer == 2) & (atom_type == 1);  % 下层石墨烯A子晶格
    hBN_B_idx = (layer == 2) & (atom_type == 2);  % 下层石墨烯B子晶格
    ...
    for idx = 1:size(translations, 1)
        ...
        % 绘制hBN氮原子 (绿色)
        scatter3(x_translated(hBN_N_idx), y_translated(hBN_N_idx), z_translated(hBN_N_idx), 100, ...
        ...
        % 绘制hBN硼原子 (黄色)
        scatter3(x_translated(hBN_B_idx), y_translated(hBN_B_idx), z_translated(hBN_B_idx), 100, ...
