function [a1, a2, atom_position] = Structure_GBNCommensuratecell(Nx, Ny, plot_structure)
    % 构建石墨烯-氮化硼超胞函数
    %
    % 输入参数:
    ...
    elseif strcmp(Type, 'AB')
        posA = (a1_single + a2_single)/3;
        posB = 2*(a1_single + a2_single)/3;
        posC = 2*(a1_BNsingle + a2_BNsingle)/3;
        posD = 3*(a1_BNsingle + a2_BNsingle)/3;
    elseif strcmp(Type, 'BA')
        posA = (a1_single + a2_single)/3;
        posB = 2*(a1_single + a2_single)/3;
        posC = 0*(a1_BNsingle + a2_BNsingle)/3;
        posD = (a1_BNsingle + a2_BNsingle)/3;
    else
        error('未知堆叠类型: %s. 支持类型: "AA" , "AB" 或 "BA" ', Type);
    end
    
    % ================== 构建超胞 ==================
    total_atoms = 2 * Nx * Ny + 2 * Nx * Ny;  % 总原子数 (每个原胞4个原子)
