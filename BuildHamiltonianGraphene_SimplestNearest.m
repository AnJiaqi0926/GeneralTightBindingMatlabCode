function [Hamiltonian, neighbor_cell, disp_vec] = BuildHamiltonianGraphene_SimplestNearest(a1, a2, atom_position, N, neighbor_cell)
    % 构建紧束缚模型哈密顿量函数
    %
    % 输入参数:
    %   a1, a2 - 正格子基矢
        ...
        parfor k = 1:N_neighbor
            ...
            for i = 1:N
                for j = 1:N
                    ...
                    % 跃迁条件：距离在合理范围内或不同层
                    if R_sq < 1.1*a_0^2 && R_sq > 0.9*a_0^2 && layer(i) == layer(j)
                        H_slice(i, j) = t1;
                    elseif R_sq < 0.1*a_0^2 && layer(i) ~= layer(j)
                        H_slice(i, j) = t2;
                    end
                end
            end
