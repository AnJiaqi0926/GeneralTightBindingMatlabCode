function [Hamiltonian, neighbor_cell, disp_vec] = BuildHamiltonianGBN(a1, a2, atom_position, N,neighbor_cell)
    % 构建紧束缚模型哈密顿量函数
    %
    % 输入参数:
    %   a1, a2 - 正格子基矢
    ...
    d_cut = (3 * a_lattice)^2; % 截断长度=3倍石墨烯晶格常数
    d_cut_min = (0.1*a_lattice)^2; %截断长度最小值
            ...
            for i = 1:N
                if k == 1
                    if sublattice(i) == 1
                        ...
                        C = -0.4277;
                        A = -0.37776;
                        H_slice(i, i) = H_slice(i, i)+Interpolation(dx(i),dy(i),A,B,C);
                    elseif sublattice(i) == 2
                        B = -0.42011;
                        C = -0.42048;
                        A = -0.36739;
                        H_slice(i, i) = H_slice(i, i)+Interpolation(dx(i),dy(i),A,B,C);
                    elseif sublattice(i) == 3
                        B = 2.3222;
                        C = 2.3476;
                        A = 2.2731;
                        H_slice(i, i) = H_slice(i, i)+Interpolation(dx(i),dy(i),A,B,C);
                    elseif sublattice(i) == 4
                        B = -1.7795;
                        C = -1.7778;
                        A = -1.8324;
                        H_slice(i, i) = H_slice(i, i)+Interpolation(dx(i),dy(i),A,B,C);
            ...
            for i = 1:N
                if k == 1
                    if sublattice(i) == 1
                        ...
                        C = -0.4277;
                        A = -0.37776;
                        Hamiltonian(i, i, k) = Hamiltonian(i, i, k)+Interpolation(dx(i),dy(i),A,B,C);
                    elseif sublattice(i) == 2
                        B = -0.42011;
                        C = -0.42048;
                        A = -0.36739;
